...
# %% [markdown]
# # Ploteos
# Ploteamos transferencia, adaptación y aislación en tres gráficos separados
...
ax2.plot(wilkinson.frequency.f/1e9, 20*np.log10(np.abs(wilkinson.s[:, 0, 0])), label='$S_{11}$', linewidth=2, color='firebrick')
ax2.plot(wilkinson.frequency.f/1e9, 20*np.log10(np.abs(wilkinson.s[:, 1, 1])), label='$S_{22}$', linewidth=2, color='forestgreen')
ax2.plot(wilkinson.frequency.f/1e9, 20*np.log10(np.abs(wilkinson.s[:, 2, 2])), label='$S_{33}$', linewidth=2, color='darkviolet')
